...
class AgentRegistry:
    ...
    def register(
        ...
        *,
        ...
        if items:
            # Re-read to get full metadata (add result may not include it)
            full = self._memory.get(items[0]["id"])
            return self._format_agent(full) if full else self._format_agent(items[0])
