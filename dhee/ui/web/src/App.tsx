...
const GLOBAL_REFRESH_MS = 15_000;
const PRODUCT_VIEWS = new Set<View>([
  "command",
  "handoff",
  "replay",
  "learnings",
...
function isProductView(view: View): boolean {
  return PRODUCT_VIEWS.has(view);
}

function isGraphOnlyView(view: View): boolean {
  return view === "canvas";
}

function isSelfHydratedView(view: View): boolean {
  return view === "router" || view === "router/sessionshistory";
}

function normalizeView(view: View | string | null): View {
  const raw = String(view || "").toLowerCase();
  if (raw === "home" || raw === "overview") return "command";
  if (raw === "memory") return "context";
  if (raw === "firewall" || raw === "context-firewall") return "router";
...
export default function App() {
  const [view, setView] = useState<View>(() => normalizeView(initialParam("view")));
  const [managerOpen, setManagerOpen] = useState(false);
  const [managerTab, setManagerTab] = useState<"workspaces" | "projects">("workspaces");
  const [tasks, setTasks] = useState<SankhyaTask[]>([]);
  const [activeTaskId, setActiveTaskId] = useState<string>(() => initialParam("task"));
  ...
  const [orgGraph, setOrgGraph] = useState<OrgGraphSnapshot | null>(null);
  ...
  const snapshotSignatures = useRef<Record<string, string>>({});

  const updateSnapshot = <T,>(
    key: string,
    next: T,
    apply: (value: T) => void
  ) => {
    ...
    if (snapshotSignatures.current[key] === signature) return;
  ...
  useEffect(() => {
    ...
    if (urlView) setView(normalizeView(urlView));
    ...
    const onPop = () => {
      ...
      setView(normalizeView(next.get("view") as View | null));
  ...
  const go = (v: View, taskId?: string) => {
    ...
    setTimeout(() => {
      ...
      const next = qs ? `?${qs}` : "";
      const hash =
        targetView === "context" && window.location.hash.startsWith("#vault")
  ...
  const renderView = () => {
    if (view === "command")
      return <CommandCenterView onNavigate={(target) => go(target)} />;
    if (view === "channel")
      return (
        <ChannelView
      ...
      return (
            ...
            >
              ...
              >
                <div>
                  ...
                  <div style={{ marginTop: 6, fontSize: 20, fontWeight: 650 }}>
        ...
        >
          <CanvasView
            ...
            orgGraph={orgGraph}
            viewer={viewer}
            onOpenVault={(teamId?: string) => {
              if (teamId) window.location.hash = `#vault/${teamId}`;
              go("context");
            }}
            onOrgGraphChanged={() => {
              void refreshOrgGraph();
    ...
    if (view === "router" || view === "router/sessionshistory")
      return <RouterView onOpenFolders={() => go("canvas")} onOpenSetup={() => go("notepad")} />;
    if (view === "handoff")
      return <HandoffHubView />;
    if (view === "replay")
      return <ProofReplayView />;
    if (view === "learnings")
      return <LearningInboxView />;
    if (view === "portability")
  ...
  return (
    <div style={{ height: "100vh", display: "flex", overflow: "hidden" }}>
      <NavRail
        view={view}
        setView={(v) => go(v)}
        conflictCount={inboxCount || conflictCount}
      />
      <div
        style={{
          flex: 1,
      ...
      >
        <TopBar
          ...
          onResetWorkspace={async () => {
            if (
              !window.confirm(
                "Reset workspace? Deletes projects, teams, folders, context items, proposals and findings for this org. Memory engrams are unaffected."
              )
            )
              return;
            try {
