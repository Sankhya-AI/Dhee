...
export interface RouterStats {
  ...
  enterpriseSavedTokens?: number;
  enterpriseSavedPct?: number;
  enterpriseRawTokens?: number;
  enterpriseSummaryTokens?: number;
  enterpriseRawFallbacks?: number;
  enterpriseGateSuggestions?: number;
  enterpriseGateDenials?: number;
  enterpriseGateFallbacks?: number;
  totalCalls: number;
  expansionRate: number;
  sessionCost: number;
...
export interface Tweaks {
  ...
  canvasStyle: "dots" | "grid" | "force" | "tree";
}
...
export interface Viewer {
  ...
  team_id?: string | null;
  team_ids: string[];
  role: "developer" | "manager" | "admin" | string;
  repo_root?: string;
}
...
export type OrgNodeType =
  ...
  | "global_team"
  | "repo"
  | string; // includes integration:slack, integration:docs, etc.

export interface OrgNode {
  id: string;
  type: OrgNodeType;
  label: string;
  health?: "healthy" | "watch" | "needs_work";
  meta?: Record<string, unknown>;
}
...
export interface ContextItem {
  ...
  tags: string[];
  status: string;
  proposal_status?: "active" | "pending_review" | "rejected" | string;
...
export interface Proposal extends ContextItem {
  proposal_status: "active" | "pending_review" | "rejected" | string;
}
...
export interface OrgGraphSnapshot {
  ...
  raw?: {
    ...
    shared_folder_paths?: string[];
    context_index: ContextItem[];
    pending_proposals: Proposal[];
    context_managers_by_team: Record<string, Record<string, unknown>>;
  };
  error?: string;
}
...
// ─── Slice-2: workspace primitive + router dashboard + context view ────────
...
export interface RouterSessionRow {
  ...
  live_usage?: {
    ...
    last_turn_cached_input_tokens?: number | null;
  ...
  active: boolean;
  task: { id?: string | null; status?: string | null };
  preview: string;
}

export interface RouterSessionsPage {
  items: RouterSessionRow[];
  next_cursor: string | null;
  active_only: boolean;
  totals: {
    tokens_saved: number;
    estimated_cost_saved_usd?: number;
    theoretical_api_value_usd?: number;
    realized_cost_saved_usd?: number;
    router_calls: number;
    sessions: number;
  };
  budget?: {
  ...
  money_math?: Record<string, string>;
}
...
export interface ContextUsageRow {
  ...
  evidence?: {
    ...
    has_direct_savings_evidence?: boolean;
  };
}

export interface ContextUsageSnapshot {
  live: boolean;
  items: ContextUsageRow[];
  totals: {
    contexts: number;
    used_contexts: number;
    usage_count: number;
    tokens_served: number;
    proven_tokens_saved: number;
    theoretical_api_value_usd: number;
...
export interface ContextEntriesSnapshot {
  repo_root: string | null;
  linked: boolean;
  repo_entries: RepoContextEntry[];
  promoted_in: {
    memory_id: string;
    memory: string;
    entry_id?: string;
  ...
  share_matrix: { repo_root: string; label: string; entry_count: number }[];
  totals?: {
    repo_entries: number;
    promoted_in: number;
    demoted_out: number;
