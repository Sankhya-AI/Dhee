...
export function ChannelView({ projectIndex, workspaceGraph, tasks, viewer, orgGraph, selectedWorkspaceId, selectedProjectId, onSelectWorkspace, onSelectProject, onSelectTask, onTasksRefresh, onOpenCanvas, onLaunchSession, onOpenManager, }) {
    const workspaces = projectIndex?.workspaces || [];
    const [continuity, setContinuity] = useState(null);
    const scopedContext = useMemo(() => {
        const all = orgGraph?.raw?.context_index || [];
        const teamId = viewer?.team_id || "";
        const projectId = viewer?.project_id || "";
        return all.filter((item) => {
            ...
            if (projectId && item.project_id === projectId)
                return true;
            if (item.scope === "user" && item.user_id === viewer?.user_id)
                return true;
            return false;
        });
    }, [orgGraph?.raw?.context_index, viewer?.project_id, viewer?.team_id, viewer?.user_id]);
    const pendingForViewer = useMemo(() => {
        const proposals = orgGraph?.raw?.pending_proposals || [];
        const teamId = viewer?.team_id || "";
        if (!teamId)
            return proposals;
        return proposals.filter((item) => !item.team_id || item.team_id === teamId);
    }, [orgGraph?.raw?.pending_proposals, viewer?.team_id]);
                    ...
                    overflow: "hidden",
                            ...
                            gap: 14,
                                    ...
                                    gap: 8,
                                            ...
                                            color: "var(--ink3)",
                                            textTransform: "uppercase",
                                        }, children: "Context scope" }), _jsx("div", { style: { fontSize: 13, color: "var(--ink)" }, children: viewer?.team_id || viewer?.project_id || viewer?.org_id || "local dhee" }), _jsxs("div", { style: {
                                            display: "flex",
                                            flexWrap: "wrap",
                                            gap: 6,
                                            fontFamily: "var(--mono)",
                                            fontSize: 9,
                                            color: "var(--ink3)",
                                        }, children: [_jsxs("span", { children: [scopedContext.length, " context items"] }), _jsx("span", { children: "\u00B7" }), _jsxs("span", { children: [pendingForViewer.length, " pending"] })] }), _jsx("button", { onClick: onOpenCanvas, style: {
                                            padding: "6px 8px",
                                            ...
                                            letterSpacing: 0.5,
                                            cursor: "pointer",
                                        }, children: "OPEN ORG MAP" })] }), _jsxs("div", { style: {
                                    padding: 12,
                                    border: "1px solid var(--border)",
                                    background: "white",
                                    display: "grid",
                                    gap: 8,
                                            ...
                                            color: "var(--ink3)",
                                            textTransform: "uppercase",
                                        }, children: "Continue context" }), _jsx("div", { style: { fontSize: 12, color: "var(--ink2)", lineHeight: 1.45 }, children: continuity?.last_session?.task_summary ||
                                            continuity?.claude_sessions?.[0]?.preview ||
                                            "No Claude session recovered yet for this repo." }), _jsxs("div", { style: {
                                            display: "flex",
                                            gap: 6,
                                            flexWrap: "wrap",
                                            fontFamily: "var(--mono)",
                                            fontSize: 9,
                                            color: "var(--ink3)",
                                        }, children: [_jsx("span", { children: continuity?.last_session?.source || "waiting" }), _jsx("span", { children: "\u00B7" }), _jsxs("span", { children: [continuity?.claude_sessions?.length || 0, " claude sessions"] })] })] }), _jsxs("div", { children: [_jsxs("div", { style: {
                                            display: "flex",
                                            justifyContent: "space-between",
                                            alignItems: "center",
                                            marginBottom: 8,
