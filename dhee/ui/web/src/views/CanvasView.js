...
function folderGraph(graph) {
    if (!graph)
        return EMPTY_LOCAL_GRAPH;
    return {
        ...EMPTY_LOCAL_GRAPH,
        ...graph,
        org_id: graph.org_id || "local",
        nodes: graph.nodes || [],
        edges: graph.edges || [],
        totals: {
            ...EMPTY_LOCAL_GRAPH.totals,
            ...(graph.totals || {}),
        },
        raw: {
            ...EMPTY_LOCAL_GRAPH.raw,
            ...(graph.raw || {}),
            mode: graph.raw?.mode || "local_context",
        },
    };
}
export function CanvasView(props) {
    return (_jsx(OrgCanvas, { graph: folderGraph(props.orgGraph), viewer: props.viewer || null, tweaks: props.tweaks, onOpenVault: props.onOpenVault || (() => { }), onOpenSession: props.onSelectSession, onChanged: props.onOrgGraphChanged || (() => { }) }));
}
function WorkspaceCanvasView({ tasks, selectedProjectId, workspaceGraph, onSelectTask, onSelectSession, onSelectWorkspace, onSelectProject, onClose, }) {
