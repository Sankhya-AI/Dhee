...
const EMPTY_LOCAL_GRAPH: OrgGraphSnapshot = {
  live: false,
  org_id: "local",
  nodes: [],
  edges: [],
...
function folderGraph(graph?: OrgGraphSnapshot | null): OrgGraphSnapshot {
  if (!graph) return EMPTY_LOCAL_GRAPH;
  return {
    ...EMPTY_LOCAL_GRAPH,
    ...graph,
...
export function CanvasView(props: CanvasViewProps) {
  return (
    <OrgCanvas
      graph={folderGraph(props.orgGraph)}
      viewer={props.viewer || null}
      tweaks={props.tweaks}
      onOpenVault={props.onOpenVault || (() => {})}
      onOpenSession={props.onSelectSession}
      onChanged={props.onOrgGraphChanged || (() => {})}
    />
  );
}

function WorkspaceCanvasView({
  tasks,
  selectedProjectId,
  workspaceGraph,
  onSelectTask,
