...
function ProductShimmer({ width = "100%", height = 12, style, }) {
...
function ProductLoadingSkeleton() {
    return (_jsxs("div", { role: "status", "aria-live": "polite", "aria-busy": "true", className: "product-grid", children: [_jsxs(Panel, { children: [_jsx("div", { className: "product-panel-label", children: "Loading Dhee state" }), _jsx("div", { className: "product-metric-grid", children: Array.from({ length: 4 }).map((_, index) => (_jsxs("div", { children: [_jsx(ProductShimmer, { width: "46%", height: 10 }), _jsx(ProductShimmer, { width: `${76 - index * 7}%`, height: 28, style: { marginTop: 12 } }), _jsx(ProductShimmer, { width: "64%", height: 10, style: { marginTop: 10 } })] }, index))) })] }), _jsx("div", { className: "product-grid product-grid--two", children: Array.from({ length: 2 }).map((_, index) => (_jsxs(Panel, { children: [_jsx(ProductShimmer, { width: "34%", height: 10 }), _jsx(ProductShimmer, { width: `${78 - index * 10}%`, height: 22, style: { marginTop: 15 } }), _jsx(ProductShimmer, { width: "92%", height: 12, style: { marginTop: 12 } }), _jsx(ProductShimmer, { width: "68%", height: 12, style: { marginTop: 8 } })] }, index))) })] }));
}
function LoadingState({ loading, error }) {
    if (loading)
        return _jsx(ProductLoadingSkeleton, {});
    if (error)
...
export function HandoffHubView() {
    ...
    return (_jsxs(Screen, { eyebrow: "HANDOFF HUB", title: "Resume without replaying the transcript.", subtitle: "Dhee turns the latest work into task state: decisions, files, blockers, commands, tests, resume confidence, and the next step.", action: _jsx("button", { onClick: refresh, style: buttonStyle, children: "refresh" }), children: [_jsx(LoadingState, { loading: loading, error: error }), data ? (_jsxs("div", { className: "product-grid product-grid--two", children: [_jsxs(Panel, { label: "CURRENT WORK", children: [_jsx("div", { style: { fontSize: 24, lineHeight: 1.15, fontWeight: 700, overflowWrap: "anywhere" }, children: currentTitle }), _jsxs("div", { style: { marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }, children: [_jsx(Pill, { tone: toneFor(get(activeSession, "state") || get(activeTask, "status")), children: String(get(activeSession, "state") || get(activeTask, "status") || "ready") }), _jsxs(Pill, { tone: "var(--green)", children: ["confidence ", Math.round(Number(get(data, "resume_confidence", 0)) * 100), "%"] }), _jsx(Pill, { children: currentRuntime }), currentModel ? _jsx(Pill, { children: currentModel }) : null, _jsx(Pill, { children: timeLabel(currentUpdated) })] }), currentCwd ? _jsx("div", { className: "product-handoff-path", children: shortPath(currentCwd) }) : null, _jsxs("div", { className: "product-handoff-stats", children: [_jsx(SmallRow, { title: "router calls", meta: compact(Number(get(activeSession, "router_calls", 0))), tone: "var(--accent)" }), _jsx(SmallRow, { title: "tokens saved", meta: compact(Number(get(activeSession, "tokens_saved", 0))), tone: "var(--green)" }), _jsx(SmallRow, { title: "task restore", meta: activeTaskId ? "linked" : "local handoff", tone: "var(--indigo)" })] })] }), _jsxs(Panel, { label: "RESUME COMMAND", children: [_jsx("div", { style: { color: "var(--ink2)", lineHeight: 1.55 }, children: "Run the handoff command when a new agent needs the current state without replaying chat." }), _jsx("pre", { style: preStyle, children: String(get(data, "command", "")) })] }), _jsxs(Panel, { label: "LATEST SAVED HANDOFF", children: [_jsx("div", { style: { fontSize: 20, lineHeight: 1.25, fontWeight: 700, overflowWrap: "anywhere" }, children: String(get(last, "task_summary", "No saved handoff yet")) }), _jsxs("div", { style: { marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }, children: [_jsx(Pill, { children: timeLabel(get(last, "updated") || get(last, "ended_at")) }), _jsx(Pill, { children: String(get(last, "agent_id", get(last, "source", "dhee"))) })] })] }), _jsx(Panel, { label: "RESUME INVENTORY", children: _jsx(MetricStack, { rows: [
                                ["tasks", tasks.length],
...
export function ProofReplayView() {
    ...
    return (_jsxs(Screen, { eyebrow: "PROOF REPLAY", title: "Replay the context decisions, not just the chat.", subtitle: "See the expansion trace: what Dhee digested, hid, expanded, injected, promoted, rejected, or derived from local records.", action: _jsx("button", { onClick: refresh, style: buttonStyle, children: "refresh" }), children: [_jsx(LoadingState, { loading: loading, error: error }), data ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "product-metric-grid", children: [_jsx(Metric, { label: "events", value: compact(get(totals, "events", rows.length)) }), _jsx(Metric, { label: "digests", value: compact(get(totals, "digests", 0)), tone: "var(--green)" }), _jsx(Metric, { label: "expansion trace", value: compact(get(totals, "expansions", 0)), tone: "var(--accent)" }), _jsx(Metric, { label: "evidence", value: compact(get(totals, "evidence", 0)), tone: "var(--indigo)" }), _jsx(Metric, { label: "derived rows", value: compact(get(totals, "derived", 0)) })] }), _jsx(Panel, { label: "DECISION TIMELINE", children: _jsx(RowList, { rows: rows, empty: "No context decisions recorded yet.", render: (row, index) => (_jsx(TimelineRow, { index: index, title: String(row.title || "Decision"), meta: `${row.source || "dhee"} - ${timeLabel(row.time)}`, detail: String(row.detail || ""), kind: String(row.kind || "event"), derived: Boolean(row.derived) }, String(row.id || index))) }) })] })) : null] }));
}
export function LearningInboxView() {
    ...
    return (_jsxs(Screen, { eyebrow: "LEARNING INBOX", title: "Only evidence-backed learnings get promoted.", subtitle: "Clear pending review candidates from agent work. Dhee should learn from success, avoided failure, repeated utility, or explicit approval.", action: _jsx("button", { onClick: refresh, style: buttonStyle, children: "refresh" }), children: [_jsx(LoadingState, { loading: loading, error: error }), data ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "product-metric-grid", children: [_jsx(Metric, { label: "candidates", value: compact(get(totals, "candidate", 0)), tone: "var(--accent)" }), _jsx(Metric, { label: "promoted", value: compact(get(totals, "promoted", 0)), tone: "var(--green)" }), _jsx(Metric, { label: "rejected", value: compact(get(totals, "rejected", 0)), tone: "var(--rose)" }), _jsx(Metric, { label: "all learnings", value: compact(get(totals, "all", rows.length)) })] }), _jsx(Panel, { label: "LEARNING REVIEW", children: _jsx(RowList, { rows: rows, empty: "No learning candidates yet.", render: (row) => {
