...
function Screen({ eyebrow, title, subtitle, children, action, }) {
    return (_jsx("div", { style: { height: "100%", overflowY: "auto", background: "var(--surface)" }, children: _jsxs("div", { style: {
                minHeight: "100%",
                padding: 24,
                display: "flex",
                ...
                gap: 18,
                        ...
                        paddingBottom: 18,
                    }, children: [_jsxs("div", { style: { maxWidth: 760 }, children: [_jsx("div", { style: { ...mono, color: "var(--accent)", marginBottom: 8 }, children: eyebrow }), _jsx("h1", { style: { fontSize: 30, lineHeight: 1.08, fontWeight: 700, letterSpacing: 0 }, children: title }), _jsx("p", { style: { marginTop: 8, color: "var(--ink2)", fontSize: 14, lineHeight: 1.55 }, children: subtitle })] }), action] }), children] }) }));
}
function Panel({ label, children, style, }) {
    return (_jsxs("section", { style: {
            ...
            minWidth: 0,
            ...style,
        }, children: [label ? _jsx("div", { style: { ...mono, color: "var(--ink3)", marginBottom: 12 }, children: label }) : null, children] }));
}
function Metric({ label, value, tone }) {
    return (_jsxs(Panel, { style: { minHeight: 94 }, children: [_jsx("div", { style: { fontSize: 26, lineHeight: 1, fontWeight: 700, color: tone || "var(--ink)" }, children: value }), _jsx("div", { style: { marginTop: 8, color: "var(--ink3)", fontSize: 12 }, children: label })] }));
}
...
function RowList({ rows, empty, render, }) {
    if (!rows.length) {
        return _jsx("div", { style: { color: "var(--ink3)", fontSize: 13 }, children: empty });
    }
    return _jsx("div", { style: { display: "grid", gap: 10 }, children: rows.map(render) });
}
...
export function ProofReplayView() {
    ...
    return (_jsxs(Screen, { eyebrow: "PROOF REPLAY", title: "Replay the context decisions, not just the chat.", subtitle: "See the expansion trace: what Dhee digested, hid, expanded, injected, promoted, rejected, or derived from local records.", action: _jsx("button", { onClick: refresh, style: buttonStyle, children: "refresh" }), children: [_jsx(LoadingState, { loading: loading, error: error }), _jsxs("div", { style: { display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 12 }, children: [_jsx(Metric, { label: "events", value: compact(get(totals, "events", rows.length)) }), _jsx(Metric, { label: "digests", value: compact(get(totals, "digests", 0)), tone: "var(--green)" }), _jsx(Metric, { label: "expansion trace", value: compact(get(totals, "expansions", 0)), tone: "var(--accent)" }), _jsx(Metric, { label: "evidence", value: compact(get(totals, "evidence", 0)), tone: "var(--indigo)" }), _jsx(Metric, { label: "derived rows", value: compact(get(totals, "derived", 0)) })] }), _jsx(Panel, { label: "DECISION TIMELINE", children: _jsx(RowList, { rows: rows, empty: "No context decisions recorded yet.", render: (row, index) => (_jsx(TimelineRow, { index: index, title: String(row.title || "Decision"), meta: `${row.source || "dhee"} - ${timeLabel(row.time)}`, detail: String(row.detail || ""), kind: String(row.kind || "event"), derived: Boolean(row.derived) }, String(row.id || index))) }) })] }));
}
export function LearningInboxView() {
    ...
    return (_jsxs(Screen, { eyebrow: "LEARNING INBOX", title: "Only evidence-backed learnings get promoted.", subtitle: "Clear pending review candidates from agent work. Dhee should learn from success, avoided failure, repeated utility, or explicit approval.", action: _jsx("button", { onClick: refresh, style: buttonStyle, children: "refresh" }), children: [_jsx(LoadingState, { loading: loading, error: error }), _jsxs("div", { style: { display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 12 }, children: [_jsx(Metric, { label: "candidates", value: compact(get(totals, "candidate", 0)), tone: "var(--accent)" }), _jsx(Metric, { label: "promoted", value: compact(get(totals, "promoted", 0)), tone: "var(--green)" }), _jsx(Metric, { label: "rejected", value: compact(get(totals, "rejected", 0)), tone: "var(--rose)" }), _jsx(Metric, { label: "all learnings", value: compact(get(totals, "all", rows.length)) })] }), _jsx(Panel, { label: "LEARNING REVIEW", children: _jsx(RowList, { rows: rows, empty: "No learning candidates yet.", render: (row) => {
                        const id = String(row.id || "");
                        const status = String(row.status || "candidate");
                        return (_jsxs("div", { style: listRowStyle, children: [_jsxs("div", { style: { minWidth: 0 }, children: [_jsxs("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }, children: [_jsx(Pill, { tone: toneFor(status), children: status }), _jsx(Pill, { children: String(row.evidence_gate || "needs approval") }), _jsx(Pill, { children: String(row.source_harness || row.source_agent_id || "agent") })] }), _jsx("div", { style: { fontSize: 17, fontWeight: 700, lineHeight: 1.25 }, children: String(row.title || id) }), _jsx("div", { style: { marginTop: 6, color: "var(--ink2)", lineHeight: 1.55 }, children: String(row.body || "") })] }), _jsxs("div", { style: { display: "flex", gap: 8, flexShrink: 0 }, children: [_jsx("button", { disabled: !id || busy === id || status === "promoted", onClick: () => act(id, "promote"), style: buttonStyle, children: "promote" }), _jsx("button", { disabled: !id || busy === id || status === "rejected", onClick: () => act(id, "reject"), style: ghostButtonStyle, children: "reject" })] })] }, id));
                    } }) })] }));
}
...
function TextList({ rows, empty }) {
    if (!rows.length)
        return _jsx("div", { style: { color: "var(--ink3)" }, children: empty });
    return (_jsx("div", { style: { display: "grid", gap: 8 }, children: rows.map((row, index) => (_jsx("div", { style: { padding: "8px 0", borderBottom: "1px solid var(--border)", color: "var(--ink2)" }, children: String(row) }, index))) }));
}
...
function SmallRow({ title, meta, tone }) {
    return (_jsxs("div", { style: { display: "flex", gap: 10, alignItems: "flex-start", padding: "7px 0", borderBottom: "1px solid var(--border)" }, children: [_jsx("span", { style: { width: 8, height: 8, marginTop: 6, background: tone || "var(--ink3)", flexShrink: 0 } }), _jsxs("div", { style: { minWidth: 0 }, children: [_jsx("div", { style: { fontSize: 13, fontWeight: 650, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: title }), _jsx("div", { style: { fontFamily: "var(--mono)", fontSize: 10, color: "var(--ink3)", marginTop: 2 }, children: meta })] })] }));
}
function TimelineRow({ index, title, meta, detail, kind, derived, }) {
    return (_jsxs("div", { style: { display: "grid", gridTemplateColumns: "44px 1fr", gap: 14, padding: "12px 0", borderBottom: "1px solid var(--border)" }, children: [_jsx("div", { style: { ...mono, color: "var(--ink3)" }, children: String(index + 1).padStart(2, "0") }), _jsxs("div", { children: [_jsxs("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 6 }, children: [_jsx(Pill, { tone: toneFor(kind), children: kind }), derived ? _jsx(Pill, { tone: "var(--accent)", children: "derived" }) : _jsx(Pill, { tone: "var(--green)", children: "recorded" }), _jsx(Pill, { children: meta })] }), _jsx("div", { style: { fontSize: 16, fontWeight: 700 }, children: title }), detail ? _jsx("div", { style: { marginTop: 4, color: "var(--ink2)", lineHeight: 1.55 }, children: detail }) : null] })] }));
}
const buttonStyle = {
