...
export function ChannelView({
  ...
  const scopedContext = useMemo(() => {
    ...
    return all.filter((item) => {
      ...
      if (teamId && item.team_id === teamId) return true;
      if (projectId && item.project_id === projectId) return true;
      if (item.scope === "user" && item.user_id === viewer?.user_id) return true;
      return false;
    });
  ...
  useEffect(() => {
    ...
    api
      .continuity()
      .then((snapshot) => {
        if (mounted) setContinuity(snapshot);
      })
      .catch(() => {
        if (mounted) setContinuity(null);
      });
    return () => {
      mounted = false;
    };
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          ...
          <StatPill
            label={`${viewer?.role || "developer"} context`}
            tone={viewer?.role === "manager" || viewer?.role === "admin" ? "var(--accent)" : "var(--indigo)"}
          />
        </div>
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 13, color: "var(--ink)" }}>
              {viewer?.team_id || viewer?.project_id || viewer?.org_id || "local dhee"}
            </div>
            <div
              style={{
                display: "flex",
            ...
            >
              <span>{scopedContext.length} context items</span>
              <span>·</span>
              <span>{pendingForViewer.length} pending</span>
            </div>
            <button
              onClick={onOpenCanvas}
              style={{
          ...
          >
            ...
            <div style={{ fontSize: 12, color: "var(--ink2)", lineHeight: 1.45 }}>
              {continuity?.last_session?.task_summary ||
                continuity?.claude_sessions?.[0]?.preview ||
                "No Claude session recovered yet for this repo."}
            </div>
            <div
              style={{
                display: "flex",
            ...
            >
              <span>{continuity?.last_session?.source || "waiting"}</span>
              <span>·</span>
              <span>{continuity?.claude_sessions?.length || 0} claude sessions</span>
            </div>
          </div>
