...
import { layoutTree, type TreeNodeIn } from "../components/canvas/treeLayout";
import type { OrgGraphSnapshot, OrgNode, Tweaks, Viewer } from "../types";
import { RepoBrainHeader } from "./ProductViews";

// Card sizes for the local-first context map.
const CARD: Record<string, { w: number; h: number }> = {
...
function cardSize(type: string): { w: number; h: number } {
  if (CARD[type]) return CARD[type];
  if (type.startsWith("integration:")) return CARD.integration;
  return CARD.default;
}

function runtimeOf(node: OrgNode): string {
  return String((node.meta || {}).runtime || "").toLowerCase();
}

function runtimeStroke(runtime: string): string {
  if (runtime === "codex") return "var(--indigo)";
  if (runtime === "claude-code" || runtime === "claude") return "var(--accent)";
  return "var(--ink3)";
}

function runtimeFill(runtime: string): string {
  if (runtime === "codex") return "var(--indigo-dim)";
  if (runtime === "claude-code" || runtime === "claude") return "var(--accent-dim)";
  return "var(--surface)";
}

function strokeFor(type: string, health?: string): string {
  if (health === "needs_work") return "var(--rose)";
  if (health === "watch") return "var(--accent)";
  if (type === "folder") return "var(--green)";
  if (type === "session") return "var(--accent)";
  if (type === "workspace") return "var(--ink2)";
  if (type === "project") return "var(--accent)";
  if (type === "team") return "var(--green)";
  if (type === "global_team") return "var(--indigo)";
  if (type === "repo") return "var(--green)";
  if (type === "integration:slack") return "var(--indigo)";
  if (type === "integration:docs") return "var(--accent)";
  if (type === "integration:email") return "var(--rose)";
  if (type === "integration:git") return "var(--green)";
  return "var(--ink3)";
}

function strokeForNode(node: OrgNode): string {
  if (node.health === "needs_work") return "var(--rose)";
  if (node.type === "session") return runtimeStroke(runtimeOf(node));
  return strokeFor(node.type, node.health);
}

function fillFor(type: string): string {
  if (type === "workspace") return "white";
  if (type === "folder") return "var(--green-dim)";
  if (type === "session") return "var(--surface)";
  if (type === "project") return "var(--accent-dim)";
  if (type === "team" || type === "global_team") return "var(--surface)";
  if (type === "repo") return "var(--green-dim)";
  if (type.startsWith("integration:")) return "var(--surface)";
  return "var(--surface)";
}

function fillForNode(node: OrgNode): string {
  if (node.type === "session") return runtimeFill(runtimeOf(node));
...
export function OrgCanvas({
  ...
  const panRef = useRef<{ down: boolean; lastX: number; lastY: number }>({
    down: false,
    lastX: 0,
    lastY: 0,
  });
  const userViewportDirtyRef = useRef(false);
  const lastAutoFitRef = useRef<{ structure: string; size: string } | null>(
    null
  );
  ...
  const { positions, bounds } = useMemo(() => {
    ...
    for (const e of graph.edges) {
      if (e.kind !== "contains" && e.kind !== "uses") continue;
    ...
    const inputs: TreeNodeIn[] = graph.nodes.map((n) => {
  ...
  const fitToBounds = () => {
    ...
    if (w <= 0 || h <= 0 || size.w <= 0 || size.h <= 0) return false;
    const padding = 80;
  ...
  useEffect(() => {
    ...
    if (!graphChanged && !sizeChanged) return;
  ...
  if (!graph.live || !hasContent) {
    return (
      ...
      >
        <RepoBrainHeader onOpenContext={() => onOpenVault()} />
        <EmptyState
          viewer={viewer}
          busy={busy}
          onAddFolder={addLocalFolder}
          manualPath={manualPath}
          onManualPathChange={setManualPath}
          onAddManualFolder={addManualFolder}
          error={pathError}
        />
      </div>
    );
    ...
    >
            ...
            return (
              ...
              >
                <rect
                  ...
                  fill={fill}
                  stroke={stroke}
                  strokeWidth={isSelected ? 2.4 : n.type === "workspace" ? 1.6 : 1.2}
                />
                {n.type === "workspace" ? (
                  <text
                    x={p.x}
                ...
                <text
                  ...
                  textAnchor="middle"
                  fontFamily="var(--font)"
                  fontSize={n.type === "workspace" ? 16 : 12}
                  fontWeight={n.type === "workspace" ? 500 : 400}
                  fill="var(--ink)"
                  pointerEvents="none"
                >
                  {truncate(n.label, n.type === "workspace" ? 30 : 22)}
                </text>
                {n.type !== "workspace" ? (
                  <text
                    x={p.x}
      ...
      {isLocalContext ? (
        ...
        >
          ...
          >
            {busy === "add-folder" ? "ADDING..." : "ADD FOLDER"}
          </button>
...
function nodeTypeLabel(node: OrgNode): string {
  const t = node.type;
  if (t === "session") {
    const runtime = runtimeOf(node);
    if (runtime === "codex") return "CODEX";
    if (runtime === "claude-code" || runtime === "claude") return "CLAUDE CODE";
  }
  if (t.startsWith("integration:")) return t.split(":")[1].toUpperCase();
...
function FooterStats({ graph }: { graph: OrgGraphSnapshot }) {
  if (graph.raw?.mode === "local_context") {
    return (
      ...
      >
        {graph.totals.folders || graph.totals.repos} folders ·{" "}
        {graph.totals.sessions || graph.totals.teams} sessions ·{" "}
        {graph.totals.shared_folders || 0} shared
      </div>
    );
  }
  return (
    ...
    >
      {graph.totals.projects} projects · {graph.totals.teams} teams ·{" "}
      {graph.totals.repos} folders · {graph.totals.pending_proposals} pending
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {viewer?.user_id ? `local · ${viewer.user_id}` : "local context"}
        </div>
        <div
          style={{
            fontSize: 18,
        ...
        >
          Dhee will show local Claude Code and Codex sessions grouped by folder.
          Add any folder you want to share context with the rest of your local
          agent folders.
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button
            ...
            style={{
              ...
              padding: "8px 14px",
              background:
                busy === "add-folder" ? "var(--surface)" : "var(--accent-dim)",
              color: "var(--accent)",
              border: "1px solid var(--accent)",
              borderRadius: 4,
              cursor: busy === "add-folder" ? "wait" : "pointer",
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") void onAddManualFolder();
              }}
            ...
            <button
              ...
              disabled={!manualPath.trim() || busy === "manual-folder"}
              style={{
                fontFamily: "var(--mono)",
                fontSize: 11,
                padding: "8px 12px",
            ...
            >
              {busy === "manual-folder" ? "LINKING..." : "LINK PATH"}
            </button>
          </div>
          <code
            style={{
          ...
          {error ? <div style={{ fontSize: 11, color: "var(--rose)" }}>{error}</div> : null}
        </div>
      </div>
    </div>
  );
