...
export function WorkspaceManagerModal({ open, onClose, projectIndex, initialWorkspaceId, initialTab = "workspaces", onChanged, }) {
    ...
    useEffect(() => {
        ...
        setProjectEdits((prev) => {
            ...
            for (const project of currentWorkspace.projects || []) {
                const folders = (project.scopeRules || []).map((r) => r.pathPrefix);
    ...
    if (!open)
        return null;
    const pickFolder = async (onPicked, prompt) => {
        setError(null);
        try {
    ...
    const onUpdateProject = (project) => () => runGuarded("Project updated.", async () => {
        ...
        const foldersChanged = currentFolders.length !== draftFolders.length ||
                                                ...
                                                background: "rgba(203,63,78,0.04)",
                                                        ...
                                                        return (_jsxs("div", { style: {
                                                                ...
                                                                gap: 8,
                                                                                ...
                                                                                fontSize: 10,
                                                                                color: "var(--ink3)",
                                                                            }, children: "runtime" }), _jsx("select", { value: draft.defaultRuntime, onChange: (e) => setDraft({ defaultRuntime: e.target.value }), style: { ...inputStyle, flex: 1 }, children: runtimeOptions.map((option) => (_jsx("option", { value: option, children: option }, option))) })] }), _jsx(FoldersEditor, { folders: draft.folders, onChange: (folders) => setDraft({ folders }), onPick: (apply) => void pickFolder(apply, `Add a folder to ${project.name}`), disabled: busy }), _jsxs("div", { style: { display: "flex", justifyContent: "space-between", gap: 8 }, children: [_jsx("button", { onClick: () => void onDeleteProject(project)(), disabled: busy, style: { ...buttonDanger, opacity: busy ? 0.5 : 1 }, children: "delete" }), _jsx("button", { onClick: () => void onUpdateProject(project)(), disabled: busy, style: { ...buttonPrimary, opacity: busy ? 0.5 : 1 }, children: "save" })] })] }, project.id));
                                                    }) }))] })] }))] }))] }), _jsxs("div", { style: {
                        padding: "10px 20px",
                        borderTop: "1px solid var(--border)",
                        background: "var(--bg)",
...
function FoldersEditor({ folders, onChange, onPick, disabled, }) {
