...
export function Markdown({ source, wikiResolve }) {
    ...
    while (i < lines.length) {
        ...
        if (/^```/.test(line)) {
            ...
            i += 1;
            parts.push(`<pre class="md-pre" data-lang="${escape(lang)}"><code class="md-code">${escape(buf.join("\n"))}</code></pre>`);
            continue;
        }
        if (/^### /.test(line)) {
        ...
        if (/^>\s?/.test(line)) {
            ...
            parts.push(`<blockquote class="md-quote">${renderInline(buf.join(" "), wikiResolve)}</blockquote>`);
            continue;
        }
        if (/^\s*[-*]\s/.test(line)) {
            const items = [];
            while (i < lines.length && /^\s*[-*]\s/.test(lines[i])) {
                items.push(`<li>${renderInline(lines[i].replace(/^\s*[-*]\s/, ""), wikiResolve)}</li>`);
                i += 1;
            }
        ...
        if (/^\s*\d+\.\s/.test(line)) {
            ...
            while (i < lines.length && /^\s*\d+\.\s/.test(lines[i])) {
                items.push(`<li>${renderInline(lines[i].replace(/^\s*\d+\.\s/, ""), wikiResolve)}</li>`);
                i += 1;
            }
        ...
        parts.push(`<p class="md-p">${renderInline(buf.join("\n"), wikiResolve)}</p>`);
    }
    return (_jsx("div", { className: "md-root", style: {
            fontFamily: "var(--font)",
            fontSize: 13.5,
            lineHeight: 1.55,
            color: "var(--ink)",
