...
// ---------------------------------------------------------------------------
// Deterministic hierarchical canvas layout.
//
// Our graph has real semantics: workspace → projects → (sessions, tasks,
// broadcasts, results, files, assets). Force-directed jiggles hide that
// structure. We instead assign coordinates by walking the hierarchy:
//
//   - workspace at origin
//   - projects on a horizontal band below the workspace (evenly spaced)
//   - each project has a column of children grouped by type
//
// Unrelated nodes (no workspace/project parent in the edge list) are
// placed in an overflow strip at the bottom so they are still visible
...
export interface LaidOutGraph {
  nodes: Array<WorkspaceGraphNode & NodeLayout>;
...
function linkEndpoint(endpoint: unknown): string {
  if (!endpoint) return "";
...
interface ChildMap {
  byParent: Map<string, Map<string, string[]>>; // parentId → type → [childId]
}
...
function buildChildMap(
  ...
  for (const link of links) {
    const src = linkEndpoint((link as { source?: unknown }).source);
    const tgt = linkEndpoint((link as { target?: unknown }).target);
...
export function layoutGraph(
  ...
  const rootAnchors: { id: string; x: number; y: number }[] = [];
  ...
  const projectsByRoot = new Map<string, typeof projects>();
  ...
  const projectBottoms = new Map<string, number>(); // projectId → y of its child start
  for (const anchor of rootAnchors) {
    const list = projectsByRoot.get(anchor.id) || [];
    if (list.length === 0) continue;
    const projectWidths = list.map((p) => size(p.type).w);
    const totalWidth =
      projectWidths.reduce((sum, w) => sum + w, 0) + PROJECT_X_GAP * (list.length - 1);
  ...
  for (const [parentId, typesMap] of childMap.byParent) {
    ...
    const columns: Array<{ type: string; ids: string[]; w: number }> = [];
    ...
    for (const [type, ids] of typesMap) {
      ...
      if (columns.some((col) => col.type === type)) continue;
      columns.push({ type, ids, w: size(type).w });
    }
  ...
  if (orphans.length) {
    ...
    const strip = orphans.map((o) => placed.get(o.id)).filter(Boolean) as NodeLayout[];
    if (strip.length) {
      const totalW = strip[strip.length - 1].x + strip[strip.length - 1].width - strip[0].x;
      const shift = -totalW / 2 - strip[0].x;
      for (const laid of strip) laid.x += shift;
    }
  }
