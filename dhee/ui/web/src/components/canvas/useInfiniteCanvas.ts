...
export function useInfiniteCanvas({
  ...
  const animateTo = useCallback(
    (target: CanvasState, duration: number = 320) => {
      cancelAnimation();
  ...
  const springBackIfNeeded = useCallback(() => {
    ...
    if (vpRight < bLeft) newPanX = -(bLeft - vpW / cur.zoom) * cur.zoom;
    else if (vpLeft > bRight) newPanX = -bRight * cur.zoom;
    if (vpBottom < bTop) newPanY = -(bTop - vpH / cur.zoom) * cur.zoom;
    else if (vpTop > bBottom) newPanY = -bBottom * cur.zoom;
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const isInteractive = t.closest(
        "button, a, input, textarea, select, [data-canvas-draggable], [data-no-pan]",
      );
      if (isInteractive && !spaceRef.current) return;
      e.preventDefault();
      cancelAnimation();
      cancelInertia();
      setIsPanning(true);
  ...
  const handleMouseUp = useCallback(() => {
    ...
    setIsPanning(false);
    if (wasPanning && !didInertia) springBackIfNeeded();
  ...
  const fitToCards = useCallback(
    (
      ...
      opts?: { maxZoom?: number; minZoom?: number; animate?: boolean },
    ) => {
      cancelAnimation();
  ...
  const actions = useMemo(
    () => ({ zoomIn, zoomOut, resetZoom, fitToCards, animateTo, cancelAnimation, setState }),
    [zoomIn, zoomOut, resetZoom, fitToCards, animateTo, cancelAnimation],
  );
...
export type CanvasActions = ReturnType<typeof useInfiniteCanvas>["actions"];
